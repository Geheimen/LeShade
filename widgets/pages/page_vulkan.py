...
class PageVulkan(QWidget):
    def __init__(self, game_param: str | None = None):
        ...
        label_description = QLabel(
            f"<html><strong>{self.game_name}</strong> uses Vulkan as rendering api, so you need to set environment variables on steam launch options, heroic games launcher or whatever the launhcer you use.</hmtl>")
    ...
    def copy_command(self, is_steam: bool) -> None:
        if is_steam:
            ...
            self.clipboard.setText('WINEDLLOVERRIDES=vulkan-1=n')
